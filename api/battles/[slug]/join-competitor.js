...
export default async function handler(req, res) {
  try {
    ...
    if (doc.status !== "enrolling")
      return res.status(400).json({ message: "Signups closed" });

    const exists = doc.competitors.find(
      n => n.toLowerCase() === clean.toLowerCase()
    );
    if (exists)
      return res.status(400).json({ message: "Name already in list" });
    ...
    console.error("join-competitor error:", e);
